...
\begin{tikzpicture}[
    ...
    \draw[platform] (-7.5, -4.5) rectangle (6.5, 6.5);
    % \begin{scope}
    ...
    \draw[kinematic_arc] ($(Origin) + (-90:4.2)$) arc (-90:16:5);
    ...
    \draw[data_line] (Arduino.east) -- ++(1,0) |- node[xshift=-0.1cm, yshift=0.2cm, font=\scriptsize\bfseries] {Logic} (Breadboard.west);

    \draw[power_line] (Breadboard.south) -- node[right, font=\scriptsize\bfseries] {Motor Phases} (Stepper.north);

    \draw[data_line] (Pi.south) |- node[pos=0.75, above, font=\scriptsize\bfseries] {Pi Camera cable} (BoxB.west);
