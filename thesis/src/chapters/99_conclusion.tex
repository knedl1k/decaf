...
\subsection{Mechanical Handling and Kinematics}
Operational testing revealed several mechanical limitations. First, occasional multi-card pickups were observed, particularly when the end-effector interacted with the top of a densely packed stack. While the excess cards typically detached during transit due to gravity without jamming the system, future iterations should implement passive mechanical separation features in the input hopper (such as angled walls or friction pads) to ensure strict single-card feeding.

Second, the vertical travel of the Z-axis mechanism is currently constrained by the maximum rotational angle of the micro servo. As the input stack depletes, the end-effector lacks the necessary stroke length to reach the bottom of the hopper. Replacing the servo-driven linkage with a continuous linear actuator would provide the full vertical range required to process high-capacity stacks. 

Lastly, as noted in \Cref{chap:sw}, the current target-seeking loop for the main axis causes abrupt decelerations. Implementing a full Proportional-Integral-Derivative (PID) controller is necessary to achieve critically damped settling. This will eliminate mechanical vibrations during the camera inspection phase and prevent cards from accidentally detaching from the suction cup due to sudden inertial shifts.

\subsection{Microcontroller Optimization}
